...
function getLatLng(location) {
	...
	geocoder.geocode( { address: address }, function(results, status) 
	{
		if (status == google.maps.GeocoderStatus.OK) 
		{
			var thislatlng = results[0].geometry.location;
...
function addResults(json) 
{
	...
	if (json.max_id == last_id && pos_changed == false && dis_changed == false && q_changed == false)
	{
	...
	else
	{
		...
		if (json.results && json.results.length) 
		{
			...
			// update status text
			$('#status').html('Found '+ json.results.length + ' tweet' + ((json.results.length > 1) ? "s" : "") + ' in the last ' + interval + ' seconds... <a href="javascript:void(0)" onclick="pause()">pause</a> | <a href="javascript:void(0)" onclick="cleardata()">clear</a>' );
...
*/
				...
				html.push('"></a>');
				html.push('<abbr class="timeago" title="'+tweet.created_at+'">'+$.timeago(tweet.created_at)+'</abbr><br>');
